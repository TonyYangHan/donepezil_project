...
def calculate_ratio(root_path, fad_path, nadh_path, suffix, save,
    ...
    if save:
        ...
        out_dir = save_dir or root_path
        os.makedirs(out_dir, exist_ok=True)
        save_ratio_image(ratio, out_dir, base + ".png", LOW_PCT_DEFAULT, HIGH_PCT_DEFAULT, gamma, cbar)
        tiff.imwrite(os.path.join(out_dir, base + ".tiff"), ratio.astype(np.float32))
    return ratio
...
def process_condition(root, enable_redox=True, enable_unsat=True, enable_turnover=False,
    ...
    for roi_id in roi_ids:
        ...
        for region_name, region_mask in region_masks.get(roi_id, {}).items():
    ...
    return data_ratios, data_medians
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=args.workers) as ex:
        ...
        for fut in concurrent.futures.as_completed(future_map):
            cond = future_map[fut]
            region_ratios, region_medians = fut.result()
    ...
    for region in REGION_NAMES:
        for metric in metrics:
            ...
            print_pairwise(pairwise, f"{metric} (block medians, {region})")
            p_map = {k: v[0] for k, v in pairwise.items()}
            label_prefix = f"{metric}_{region}"
            plot_violins_all(blocks, args.conds, p_map, args.out, "Block-median t-test", ylabels.get(metric, metric), label_prefix, hide_ns, pdf_pages)
            ...
            med_map = {cond: collect_pixel_medians(cond_region_medians[key][cond]) for cond in args.conds}
            med_pairwise = pairwise_tests(med_map)
            print_pairwise(med_pairwise, f"{metric} (pixel medians, {region})")
